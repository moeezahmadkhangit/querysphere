...
**A room id is not a capability.** Direct message ids are derived from the pair that owns them (`dm_<sorted user ids>`), and user ids are handed out by search and by the suggestion list — so the id of somebody else's conversation is guessable on purpose, and the access check is the only thing in the way. `npm run test:access` in `backend/` is an adversarial test that makes a signed-up outsider guess those ids and try every surface that accepts one: both REST reads, the room list, the socket join, sending, reacting, deleting, marking read, joining a community she was not invited to, treating a DM as a community, and joining the `user:<id>` channel that carries somebody's private notifications. 28 assertions, and it starts its own server on a temp store, so it is safe to run anywhere.

Writing is revoked separately from reading. `canPost` re-checks the friendship on every write to a direct message, because friendship used to gate only the *creation* of one — removing somebody dropped them from your list and otherwise changed nothing, leaving them an open channel to you. Now the conversation stays readable to both sides, and neither can add to it. The room reports `readOnly` so the composer can say why it is closed rather than accepting a message and dropping it.
